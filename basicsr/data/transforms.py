...
def tripled_random_crop(img_gts, img_lqs, img_adds, gt_patch_size, scale, gt_path=None):
    """Paired random crop. Support Numpy array and Tensor inputs.
    ...
    if input_type == 'Tensor':
        img_lqs = [v[:, :, top:top + lq_patch_size, left:left + lq_patch_size] for v in img_lqs]
        img_adds = [v[:, :, top:top + lq_patch_size, left:left + lq_patch_size] for v in img_adds]
    else:
        img_lqs = [v[top:top + lq_patch_size, left:left + lq_patch_size, ...] for v in img_lqs]
        img_adds = [v[top:top + lq_patch_size, left:left + lq_patch_size, ...] for v in img_adds]
    ...
    if len(img_lqs) == 1:
        img_lqs = img_lqs[0]
        img_adds = img_adds[0]
    return img_gts, img_lqs, img_adds

def four_random_crop(img_gts, img_lqs, img_adds, img_deps, gt_patch_size, scale, gt_path=None):
    """Paired random crop. Support Numpy array and Tensor inputs.
    ...
    if input_type == 'Tensor':
        img_lqs = [v[:, :, top:top + lq_patch_size, left:left + lq_patch_size] for v in img_lqs]
        img_adds = [v[:, :, top:top + lq_patch_size, left:left + lq_patch_size] for v in img_adds]
        img_deps = [v[:, :, top:top + lq_patch_size, left:left + lq_patch_size] for v in img_deps]
    else:
        img_lqs = [v[top:top + lq_patch_size, left:left + lq_patch_size, ...] for v in img_lqs]
        img_adds = [v[top:top + lq_patch_size, left:left + lq_patch_size, ...] for v in img_adds]
        img_deps = [v[top:top + lq_patch_size, left:left + lq_patch_size, ...] for v in img_deps]
    ...
    if len(img_lqs) == 1:
        ...
        img_adds = img_adds[0]
        img_deps = img_deps[0]
    return img_gts, img_lqs, img_adds, img_deps
